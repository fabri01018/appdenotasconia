    ...
    if (data && data.length > 0) {
      ...
      for (const project of data) {
        try {
          ...
          await db.runAsync(
            `INSERT OR REPLACE INTO projects (id, name, updated_at, sync_status, deleted_at)
             VALUES (?, ?, ?, ?, ?)`,
            [project.id, project.name, project.updated_at, 'synced', project.deleted_at || null]
          );
        } catch (insertError) {
          ...
          if (insertError.message.includes('no such column')) {
            try {
              ...
              logSupabaseError(error2, 'inserting project', 'projects', { 
                projectId: project.id,
                projectData: project
              });
            }
          } else {
            logger.error(`❌ Failed to insert project ${project.id}:`, insertError.message);
            logSupabaseError(insertError, 'inserting project', 'projects', { 
              projectId: project.id, 
              projectData: project 
            });
