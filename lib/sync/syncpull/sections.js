...
export async function pullSectionsFromSupabase() {
  try {
        ...
        try {
          ...
          await db.runAsync(
            `INSERT OR REPLACE INTO sections (id, project_id, name, updated_at, sync_status, deleted_at)
             VALUES (?, ?, ?, ?, ?, ?)`,
            [section.id, section.project_id, section.name, section.updated_at, 'synced', section.deleted_at || null]
          );
        } catch (insertError) {
          ...
          if (insertError.message.includes('no such column')) {
            try {
              ...
              logSupabaseError(error2, 'inserting section', 'sections', {
                sectionId: section.id,
                sectionData: section
              });
            }
          } else {
            logger.error(`❌ Failed to insert section ${section.id}:`, insertError.message);
            logSupabaseError(insertError, 'inserting section', 'sections', {
              sectionId: section.id,
              sectionData: section
            });
            // Continue with next section instead of throwing
          }
        }
      }
    ...
    return { 
      success: true, 
      count: data?.length || 0,
      items: data?.map(s => ({ name: s.name, id: s.id, deleted: !!s.deleted_at })) || []
    };
  } catch (error) {
    logSupabaseError(error, 'pulling sections', 'sections', {
