 ...
 * @param {string} [specificTable] - Optional table name to sync updates for
 * @description
 ...
 */
export async function handleUpdateConflicts(specificTable = null) {
  try {
    await initDatabase();
    const db = getDb();

    console.log(`🔄 Starting update sync${specificTable ? ` for ${specificTable}` : ''}...`);

    if (specificTable) {
      await handleTableUpdates(db, specificTable);
    } else {
      // Handle updates in dependency order
    ...
    console.log(`✅ Update sync completed${specificTable ? ` for ${specificTable}` : ''}.`);
  } catch (err) {
