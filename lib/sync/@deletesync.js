 ...
 * @param {string} [specificTable] - Optional table name to sync deletions for
 * @description
 ...
 */
export async function handleDeleteConflicts(specificTable = null) {
  try {
    await initDatabase();
    const db = getDb();

    console.log(`🔄 Starting delete sync${specificTable ? ` for ${specificTable}` : ''}...`);

    const tables = specificTable ? [specificTable] : ['projects', 'tasks', 'tags', 'sections'];

    for (const table of tables) {
      await handleTableDeletions(db, table);
    }

    console.log(`✅ Delete sync completed${specificTable ? ` for ${specificTable}` : ''}.`);
  } catch (err) {
