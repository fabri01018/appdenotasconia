...
async function _initDatabase() {
  try {
    ...
    await addMissingColumns(db, 'chat_sessions', [
      { name: 'context_task_ids', type: 'TEXT' },
    ]);
    ...
    // Create filter_tags table (many-to-many relationship)
    await db.execAsync(`
    ...
    // Create filter_projects table (many-to-many relationship)
    await db.execAsync(`
    ...
    `);
    console.log('✅ Filter_projects table created');

    // Create indexes for filter relationships (for performance)
    await db.execAsync(`
      CREATE INDEX IF NOT EXISTS idx_filter_tags_filter_id ON filter_tags(filter_id);
    `);
    await db.execAsync(`
      CREATE INDEX IF NOT EXISTS idx_filter_tags_tag_id ON filter_tags(tag_id);
    `);
    await db.execAsync(`
      CREATE INDEX IF NOT EXISTS idx_filter_projects_filter_id ON filter_projects(filter_id);
    `);
    await db.execAsync(`
      CREATE INDEX IF NOT EXISTS idx_filter_projects_project_id ON filter_projects(project_id);
    `);
    ...
    try {
      ...
      console.log('✅ Filter tables verified: filters, filter_tags, filter_projects');
    } catch (error) {
      console.error('⚠️ Error verifying filter tables:', error.message);
    }
...
/**
 * Debug function to dump all database tables and their contents to the console
 */
export async function dumpDatabaseContents() {
  const db = getDb();
  try {
    console.log('\n\n================================================');
    console.log('🗄️  DATABASE DUMP START');
    console.log('================================================');
    ...
    // 2. Dump each table with formatted ASCII grid
    for (const table of tables) {
      ...
      columns.forEach(col => {
        ...
        colWidths[col] = Math.min(maxWidth, 40); // Cap width at 40
      });

      // Build header and separator
      const headerLine = columns.map(col => col.toUpperCase().padEnd(colWidths[col])).join(' | ');
      const separator = columns.map(col => '-'.repeat(colWidths[col])).join('-+-');
      
      console.log(headerLine);
      console.log(separator);

      // Build rows (truncate long text and remove internal newlines)
      rows.forEach((row) => {
        const rowLine = columns.map(col => {
          let val = String(row[col] ?? 'NULL').replace(/\n/g, ' ');
          if (val.length > colWidths[col]) {
            val = val.substring(0, colWidths[col] - 3) + '...';
          }
    ...
    console.log('\n================================================');
    console.log('🗄️  DATABASE DUMP END');
    console.log('================================================\n\n');
    return true;
  } catch (error) {
...
/**
 * Verify that filter tables exist (for testing/debugging)
 * This is a helper function to confirm database setup
 */
export async function verifyFilterTables() {
  ...
  try {
    ...
    const filterTags = await db.getAllAsync('SELECT * FROM filter_tags LIMIT 1');
    const filterProjects = await db.getAllAsync('SELECT * FROM filter_projects LIMIT 1');
    console.log('✅ Filter tables verified:', { 
      filters: filters.length, 
      filterTags: filterTags.length, 
      filterProjects: filterProjects.length 
    });
    return true;
  } catch (error) {
    console.error('❌ Error verifying filter tables:', error);
    return false;
  }
}
