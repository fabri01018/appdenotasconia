...
/**
 * Formats multiple tasks as a combined context string for AI
 * @param {Object[]} tasks - Array of task objects
 * @returns {Promise<string>} Formatted context string
 */
export async function formatMultipleTasksContext(tasks) {
  if (!tasks || tasks.length === 0) return '';
  if (tasks.length === 1) return formatTaskContext(tasks[0]);

  const sections = await Promise.all(tasks.map(t => formatTaskContext(t)));
