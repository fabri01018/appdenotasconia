...
async function getProjectByName(name) {
  return await withRetry(async () => {
    const db = getDb();
    const project = await db.getFirstAsync('SELECT * FROM projects WHERE name = ? AND deleted_at IS NULL', [name]);
    return project;
  });
}
...
async function updateProject(id, name, defaultSectionId = undefined) {
...
async function deleteProject(id) {
  ...
  return await withRetry(async () => {
    ...
    console.log(`✅ Project ${id} soft deleted locally`);
